...
export default function Home() {
  ...
  return (
    <div className="space-y-6">
        <section className="page-card grid gap-5 p-5 lg:grid-cols-[1.35fr_0.65fr] lg:p-6">
          <div>
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--accent-soft)] py-1.5 pl-2 pr-4 font-mono text-[11px] font-medium uppercase tracking-[0.04em] text-[var(--accent-h)]">
              <span className="h-1.5 w-1.5 rounded-full bg-[var(--accent)]" />
              Core em desenvolvimento
            </div>
            <h1 className="max-w-3xl text-[34px] font-semibold leading-tight text-[var(--text)] md:text-[46px]">
              Painel base para operar SaaS <b className="text-[var(--blue-l)]">multitenant</b>.
            </h1>
            <p className="mt-4 max-w-2xl text-[14.5px] leading-7 text-[var(--text2)]">
              ...
              para operar no Supabase compartilhado com os produtos acoplados.
            </p>
          ...
          <DashboardCard className="p-5">
            ...
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[var(--border-b)] bg-[var(--accent-soft)] text-[var(--accent-h)]">
                <Database size={20} strokeWidth={1.3} />
              </div>
              <div>
                <h2 className="text-[15px] font-semibold">
                  Supabase do PMT-Braidotti
                </h2>
                <p className="mt-2 text-[13.5px] leading-6 text-[var(--text2)]">
                  Core e PMT passam a compartilhar a mesma base DEV, mantendo
                  isolamento por tenant, RLS e permissões centralizadas.
                </p>
              </div>
            </div>
        ...
        <section className="grid gap-px overflow-hidden rounded-[14px] border border-[var(--border)] bg-[var(--border)] md:grid-cols-2 xl:grid-cols-4">
          {metrics.map((metric) => (
            <div className="bg-[var(--bg2)] p-5 transition hover:bg-[var(--bg3)]" key={metric.label}>
              ...
              <div className="mt-3 text-[30px] font-semibold leading-none text-[var(--text)]">
                {metric.value}
              </div>
              <div
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr_380px]">
          ...
          <DashboardCard className="p-5">
            ...
            <div className="mt-5 flex items-center gap-2 rounded-lg border border-[var(--border-b)] bg-[var(--accent-soft)] p-4 text-[13.5px] text-[var(--text2)]">
              <CheckCircle2 size={17} className="shrink-0 text-[var(--accent-h)]" />
